...
Shader "Surface Reconstruction/RippleWaveWireframe"
{
	Properties{
		...
		v2g vert(appdata_base v) {
			...
			{
				float4 convertedNorm = float4(v.normal.x, v.normal.y, v.normal.z, 0);
	...
	float4 frag(g2f i) : COLOR {
			...
			float4 color = (1 - I) * _BaseColor + I *_WireColor * 0.3;
			if (abs(i.distToTarget - rippleTravelDistance) < _RippleWidth)
				color = color + I *_WireColor * 0.7*(1 - abs(i.distToTarget - rippleTravelDistance) / _RippleWidth);
			color.a = I;
